...
public class GenerateParallelTestForDetection {
    ...
    public static void main(String[] args) {
        ...
        List<String> docNull = new ArrayList<>(), codeNull = new ArrayList<>(), locsNull = new ArrayList<>();
        int i = 0;
        while (i < doc.size()) {
            if (isNull(code.get(i)) || isTrue(code.get(i)) || isUnresolvable(code.get(i)) || isIgnoredException(code.get(i))) {
                doc.remove(i);
                code.remove(i);
                locs.remove(i);
            } else if (isNull(doc.get(i))) {
                docNull.add(doc.remove(i));
                codeNull.add(code.remove(i));
                locsNull.add(locs.remove(i));
            } else i++;
        }
        write(doc, path + "/test.doc");
        write(code, path + "/test.code");
        write(locs, path + "/test.loc");
        write(docNull, path + "/null.doc");
        write(codeNull, path + "/null.code");
        write(locsNull, path + "/null.loc");
    }
    ...
    private static boolean isIgnoredException(String s) {
        return s.startsWith("UnsupportedOperationException ");
    }
